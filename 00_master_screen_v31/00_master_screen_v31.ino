...
const char* ID = "dmt09"; 
const char* HOT_CHILI_VERSION        = "Master IoT dmt01 NIS"; 
...
const char* MQTT_TOPIC_PUB_INFO      = "dmt09/info";       // for status infos and debugging
...
const int ledCtrlPin    = D6;  // *D6*  D5   yellow cable - BLUE LED
const int buttonCtrlPin = D5;  //  D6  *D5*  white 
...
void loop() {
  ...
  if (mySec != oldSec)  // only when sec changed
  {
    ...
    {
      if (!isnan(humid) && ((int)humid > 0) && ((int)humid < 100))
      {
        sprintf(dataInfo,"%4d",(int)humid);
        client.publish(MQTT_TOPIC_PUB_AIR, dataInfo);
      }
  
      if (!isnan(temp) && ((int)temp > 0) && ((int)temp < 50))
      {
